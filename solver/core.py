...
from ingest.schema import is_client_open, is_client_closed_on
...
def solve_week(
    ...
    for i in range(n_clients):
        ...
        # ── Productivity gate ───────────────────────────────────────────
        # If a non-urgent client's best-day refill is below the productive
        # threshold, demote their disjunction penalty to "deep-future"
        # (20K). This makes the stop genuinely droppable: the solver will
        # only include it if the marginal route cost is tiny (e.g., a
        # neighbor 1 mile off the path). For everything else, defer until
        # next week when the refill has grown into the productive range.
        # Without this, every client in the horizon got a 1.5M "must-serve"
        # penalty regardless of size — leading to 100-lb stops eating
        # 18-min pump setups on already-overtime trucks.
        _min_productive = float(getattr(_cfg, 'MIN_PRODUCTIVE_LBS', 300))
        max_refill_in_horizon = max(
            (refills_by_day[d][i + 1] for d in range(_npd)),
            default=0,
        )
        if (worst_dte > URGENT_DAYS
                and max_refill_in_horizon < _min_productive
                and not client_is_mandatory[i]):
            base_penalty = min(base_penalty, 20_000)
    ...
    METRO_CROSS_PENALTY = 50_000    # ~30 mi of extra cost — strong deterrent
                                    # against cross-metro zigzags (e.g. East
                                    # → Buckeye → East). Was 8K (≈5 mi) —
                                    # too weak to prevent the solver from
                                    # crossing the metro twice in one route.
    ...
    _lateness_tiers = tuple(getattr(_cfg, 'LATENESS_DTE_TIERS', (
        (1.5, 100), (3.0, 30), (5.0, 10), (7.0, 3), (float('inf'), 1),
    )))
    if _late_penalty > 0:
        # Compute preferred day for each client based on fill economics
        client_deadline = []
        client_late_mult = []   # DTE-tiered lateness multiplier per client
        for i in range(n_clients):
            node = i + 1
            tank_lbs = max(float(pool.iloc[i]['Tank_lbs']), 1.0)
            ...
            # DTE-tiered lateness multiplier: clients close to stockout pay
            # much more per day late. See config.LATENESS_DTE_TIERS for the
            # rationale and tier values.
            dte_i = float(pool.iloc[i].get('Days_Until_Stockout') or 999.0)
            mult = 1
            for max_dte, m in _lateness_tiers:
                if dte_i <= max_dte:
                    mult = m
                    break
            client_late_mult.append(mult)
        ...
        _sweep_max_dte    = float(getattr(_cfg, 'NEIGHBOR_SWEEP_MAX_DTE', 7.0))
        _sweep_min_lbs    = float(getattr(_cfg, 'NEIGHBOR_SWEEP_MIN_LBS', 300.0))
        if _sweep_enabled and n_clients > 1:
            ...
            for i in range(n_clients):
                ...
                # DTE gate: don't pull a client who doesn't actually need oil.
                # If they have >7 days of runway, the rolling horizon will
                # catch them naturally on a future, better-fitted route.
                if d2sos[i] > _sweep_max_dte:
                    continue
                ...
                for j in range(n_clients):
                    ...
                    if d2sos[i] < j_day:
                        continue
                    # Fill + size economics on j's day — both gates required:
                    #   1) Tank must be at least 50% empty (room for delivery)
                    #   2) Refill must be at least MIN_LBS (worth the 18-min setup)
                    # A 100-lb pull at 50% fill is uneconomic; a 600-lb pull
                    # at 60% fill IS worth it. Both axes matter.
            ...
            if n_pulled:
                print(f"  Neighbor-sweep: pulled {n_pulled} client(s) to earlier "
                      ...
                      f"max_dte={_sweep_max_dte:.0f}d, "
                      f"min_lbs={_sweep_min_lbs:.0f})")
                ...
                def _cb(from_idx, to_idx):
                    ...
                    if tn != 0:  # not depot
                        ...
                        if ci < _n_clients:
                            ...
                            # Per-client multiplier from DTE tiers (see
                            # config.LATENESS_DTE_TIERS). Clients close to
                            # stockout pay 30-100x base; safe clients pay 1x.
                            # This eliminates the binary cliff that previously
                            # treated DTE=2.5 identically to DTE=50.
                            days_late = max(0, _day - _deadlines[ci])
                            late_mult = _late_mult_per_client[ci]
    ...
    for v in range(n_vehicles):
        ...
        while not routing.IsEnd(idx):
            ...
            if model_node != 0:
                ...
                # Fill_Pct = fraction of tank filled by THIS delivery on the
                # actual visit day. Always recompute from the visit-day refill
                # — DO NOT use pool_row['Fill_Pct'], which holds the internal
                # end-of-horizon sizing hint (can be ~100% even when the
                # actual refill is much smaller). That mismatch was causing
                # schedule rows like "Refill 484 / Fill 97%" to appear
                # inconsistent.
                fill_pct = refill / tank_lbs if tank_lbs > 0 else 0
                ...
                route_stops.append({
                    ...
                    # Current_lbs displayed = projected level at the moment
                    # the truck arrives (Tank − Refill). NOT today's level.
                    # This keeps the row internally consistent: every column
                    # — Tank, Current, Refill, Fill_Pct — refers to the
                    # actual visit day. Showing today's Current_lbs caused
                    # confusion because "1050 − 741 = 309 free" didn't match
                    # the 484 refill (which projects forward to Saturday).
                    'Current_lbs':         max(int(tank_lbs - refill), 0),
                    'Refill_lbs':          refill,
