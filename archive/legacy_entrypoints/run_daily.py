...
def run_daily(
    ...
    # Read live tank levels from the Query sheet in SK_Delivery_System.xlsx.
    # Falls back to anova_latest.json if Query sheet is unavailable.
    try:
        from anova_fetch import load_anova_from_query_sheet
        query_readings = load_anova_from_query_sheet(INPUT_FILE)
        n_overrides = 0
        if query_readings:
            for cid, reading in query_readings.items():
                if reading.get('confidence') == 'sensor':
                    state[cid] = float(reading['level_lbs'])
                    n_overrides += 1
        if not query_readings:
