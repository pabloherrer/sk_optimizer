...
def save_excel_schedule(
    ...
    # Group by (Truck, Date) — NOT (Truck, Day). The horizon spans multiple
    # weeks, so two Fridays / two Saturdays exist; grouping by weekday name
    # collapsed them into one row, doubling stops/lbs and producing fake
    # Cap_Pct values like 116% or 150%.
    if not combined.empty:
        rows = []
        group_keys = ['Truck', 'Date'] if 'Date' in combined.columns else ['Truck', 'Day']
        for keys, grp in combined.groupby(group_keys):
            truck = keys[0]
            day = grp['Day'].iloc[0] if 'Day' in grp.columns else (
