...
# ─────────────────────────────────────────────────────────────────────────────
# Single consolidated ingest path (Query sheet → state)
# ─────────────────────────────────────────────────────────────────────────────

def load_readings_from_query_sheet(excel_path: Path) -> Dict[str, TankReading]:
    """
    Read live Anova readings from the Query sheet of SK_Delivery_System.xlsx.

    The Query sheet is populated by Power Query from the Azure API.
    Customer names like 'HAR - 8031 - HAROLDS' contain the SK client ID.

    Returns {client_id: TankReading}.
    """
    ...
    try:
        ...
        log.warning('Cannot open %s: %s', excel_path.name, e)
        return {}
    ...
    for r in range(2, ws.max_row + 1):
        ...
        product = row[col_idx.get('product', 6)]

        if not device_id or not customer:
            continue
        try:
...
def apply_query_sheet_to_state(
    *,
    state,
    excel_path: Path,
    clients_df: pd.DataFrame,
    rate_lookup: Optional[Dict[str, float]] = None,
    fresh_hours: float = 24.0,
    stale_max_hours: float = 72.0,
) -> Tuple[set, dict]:
    """
    Single entry point for Anova → state ingest.

    Reads live tank levels from the Query sheet of SK_Delivery_System.xlsx
    and overrides estimated state with sensor data. Stale readings (24-72h)
    are PROJECTED FORWARD by consumption rate instead of being thrown away,
    because a 42h-stale sensor projection is far better than a 5-day-stale
    estimate.

    Parameters
    ----------
    state           : InventoryState — mutated in place
    excel_path      : path to SK_Delivery_System.xlsx (Query sheet inside)
    clients_df      : the clients DataFrame (used for fallback rate lookup)
    rate_lookup     : optional {client_id: lbs/day}; built from clients_df if None
    fresh_hours     : readings ≤ this are 'sensor' (use as-is)
    stale_max_hours : readings up to this are 'sensor-projected' (project forward)

    Returns (monitored_ids, summary_dict).
    """
    from state.manager import ClientState

    readings = load_readings_from_query_sheet(excel_path)
    if not readings:
        return set(), {'fresh': 0, 'projected': 0, 'skipped': 0, 'total': 0}

    if rate_lookup is None:
        rate_lookup = {}
        for _, r in clients_df.iterrows():
            cid = str(r['ID'])
            rate = r.get('Avg_LbsPerDay')
            if rate is not None and pd.notna(rate) and rate > 0:
                rate_lookup[cid] = float(rate)

    monitored_ids: set = set()
    n_fresh = n_projected = n_skipped = 0

    for cid, rec in readings.items():
        # age_hours computed lazily (uses 'now' at call time, not load time)
        ts = rec.timestamp.tz_localize(None) if rec.timestamp.tz else rec.timestamp
        now = pd.Timestamp.now(tz='UTC').tz_localize(None)
        age_h = max((now - ts).total_seconds() / 3600.0, 0.0)

        if age_h <= fresh_hours:
            final_lbs = rec.level_lbs
            confidence = 'sensor'
            n_fresh += 1
        elif age_h <= stale_max_hours:
            rate = rate_lookup.get(cid, 0.0)
            days_since = age_h / 24.0
            final_lbs = max(0.0, rec.level_lbs - rate * days_since)
            confidence = 'sensor-projected'
            n_projected += 1
        else:
            n_skipped += 1
            continue
    ...
    summary = {
        ...
        'skipped': n_skipped,
        'total': n_fresh + n_projected,
    }
    return monitored_ids, summary
