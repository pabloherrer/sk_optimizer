...
from ingest.matrix import load_matrix
from ingest.schema import (
    load_time_windows, load_closures, load_depot_config, load_trucks,
)
from solver.core import solve_horizon
from reporting.writers import save_excel_schedule, save_route_map
...
from forecast.demand_model import fit_demand_models, attach_demand_columns
from forecast.safety_stock import build_urgency_profiles, attach_urgency_columns
from solver.economics import CostModel, DEFAULT_COSTS
from solver.objective import (
    calibrate_legacy_knobs, cost_units_per_dollar, explain_objective_in_dollars,
)
from state.warm_start import plan_overlap, shift_plan_for_today, _vehicle_index
from ingest.anova import (
    load_all_readings, apply_anova_to_state, tighten_sigma_for_monitored,
)
from reporting.diagnostics import compute_plan_quality, pretty_print_quality
from reporting.smartservice import export_smartservice_csv
...
def main() -> int:
    ...
    if args.today:
        ...
        # Plan starting from TOMORROW — the optimizer runs during the day
        # to produce routes for the next delivery day, not the current one.
        today = (pd.Timestamp.today() + pd.Timedelta(days=1)).normalize()
    ...
    from forecast.consumption import estimate_consumption_rates as _legacy_rates
    ...
    if not state.clients:
        ...
        from state.manager import ClientState
    ...
    # ── ANOVA live-inventory override ──────────────────────────────────────
    # Single consolidated path: Query sheet → state. Stale (24-72h) readings
    # are projected forward by consumption rate, not discarded.
    from ingest.anova import apply_query_sheet_to_state
    monitored_ids, anova_summary = apply_query_sheet_to_state(
        state=state, excel_path=input_file, clients_df=clients_df,
    )
    if anova_summary['total']:
        print(f'\n  ANOVA: {anova_summary["total"]} client(s) using sensor data '
              f'({anova_summary["fresh"]} fresh ≤24h, '
              f'{anova_summary["projected"]} projected from 24-72h stale)')
        if anova_summary['skipped']:
            print(f'         ⚠ {anova_summary["skipped"]} sensor reading(s) '
                  f'> 72h old — using estimate fallback')
        tighten_sigma_for_monitored(
            models=models, monitored_client_ids=monitored_ids,
            sigma_multiplier=0.4,
        )
    else:
        print('\n  ANOVA: no readings in Query sheet (Power Query may need refresh)')

    # Legacy --enable-anova fallback (CSV/Excel files) — kept for backfill scenarios
    if args.enable_anova and not monitored_ids:
        if ANOVA_CSV_FILE.exists() or ANOVA_EXCEL_FILE.exists():
            ...
            if readings:
                ...
                if applied:
                    print(f'  ANOVA fallback: applied {len(applied)} readings '
                          f'from CSV/Excel files.')
                    tighten_sigma_for_monitored(
                        models=models, monitored_client_ids=monitored_ids,
                        sigma_multiplier=0.4,
                    )
    ...
    # The IRP-calibrated per-client drop penalties (build_per_client_disjunction_penalties)
    # were never wired into solve_horizon. The legacy solver uses hand-tuned
    # magic constants (1B for must-visit, 100M for critical, 5M for urgent)
    # which produce the right hard/soft rankings via the urgency tier columns
    # we already attach. See ADR-003 for the decision to remove that dead
    # computation. The chance-constrained urgency profiles ARE the value-add;
    # they flow through `attach_urgency_columns` into the solver via the
    # `Urgency` and `Days_Until_Stockout` columns on clients_df.
